...
# STAGING Kubernetes Cluster Initialization Script
# Run this script ONLY on k8s-stg1 (first control plane)
...
banner() {
    echo -e "${BLUE}"
    echo "=============================================================="
    echo "    STAGING Kubernetes Cluster Initialization - k8s-stg1"
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script initializes the first control plane node for a"
    echo "2-node High Availability Kubernetes cluster."
    echo
}
...
localAPIEndpoint:
  advertiseAddress: ${CONTROL_PLANES[k8s-stg1]}
  bindPort: 6443
nodeRegistration:
...
create_storage_directories() {
    log "Creating storage directories on k8s-stg1..."
...
#!/bin/bash
# Join command for additional control planes (k8s-stg2)
...
EOF
    ...
    if curl -k "https://${CONTROL_PLANES[k8s-stg1]}:6443/healthz" &>/dev/null; then
...
show_completion_info() {
    echo
    echo -e "${GREEN}=============================================================="
    echo "🎉 STAGING Kubernetes Cluster Initialization Completed!"
    echo -e "==============================================================${NC}"
    echo
    ...
    echo "• control-plane-join.sh - For k8s-stg2"
    ...
    echo
    echo "=== Next Steps ==="
    echo "1. Copy /opt/kubernetes/join-info/control-plane-join.sh to k8s-stg2"
    echo "2. Run 04-ha-cluster-join.sh on k8s-stg2"
    ...
    echo
    echo -e "${GREEN}k8s-stg1 initialization completed successfully!${NC}"
...
main() {
    ...
    log "Starting STAGING Kubernetes cluster initialization on k8s-stg1..."
    ...
    success "STAGING Kubernetes cluster initialization completed successfully!"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script initializes the first control plane of an STAGING Kubernetes cluster."
        echo "Run this script ONLY on k8s-stg1 after load balancer setup."
        exit 0
        ;;
    *)
        main "$@"
