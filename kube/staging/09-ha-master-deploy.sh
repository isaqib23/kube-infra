...
# STAGING Kubernetes Master Deployment Orchestration Script
# Run this script to orchestrate the complete HA cluster deployment
# Purpose: Coordinate deployment across all 2 Dell R740 servers
...
banner() {
    ...
    echo "################################################################"
    echo "#                                                              #"
    echo "#     STAGING Kubernetes Cluster Master Deployment Script     #"
    echo "#     2x Dell PowerEdge R740 Servers                          #"
    ...
    echo "• Phase 4: Cluster initialization (k8s-stg1)"
    echo "• Phase 5: Control plane joining (k8s-stg2)"
...
check_prerequisites() {
    ...
    if [[ "$hostname" != "k8s-stg1" ]]; then
        error "This orchestration script should only be run from k8s-stg1. Current hostname: $hostname"
    fi
...
get_deployment_configuration() {
    ...
    echo
    read -p "This will deploy a complete STAGING Kubernetes cluster. Continue? [y/N]: " CONFIRM_DEPLOY
...
phase_server_preparation() {
    ...
    log "Preparing all 2 Dell R740 servers..."
    
    # Execute on k8s-stg1 first (local)
    log "Running server preparation on k8s-stg1 (local)..."
    if "$SCRIPT_DIR/01-server-preparation.sh"; then
        success "Server preparation completed on k8s-stg1"
    else
        error "Server preparation failed on k8s-stg1"
    fi
...
phase_loadbalancer_setup() {
    ...
    for node in "${!CONTROL_PLANES[@]}"; do
        if [[ "$node" == "k8s-stg1" ]]; then
            log "Running load balancer setup on k8s-stg1 (local)..."
            if "$SCRIPT_DIR/02-ha-loadbalancer-setup.sh"; then
                success "Load balancer setup completed on k8s-stg1"
            else
                error "Load balancer setup failed on k8s-stg1"
            fi
        else
...
phase_cluster_initialization() {
    ...
    log "Initializing Kubernetes cluster on k8s-stg1..."
    
    if "$SCRIPT_DIR/03-ha-cluster-init.sh"; then
        success "Cluster initialization completed on k8s-stg1"
    else
        error "Cluster initialization failed on k8s-stg1"
    fi
...
show_deployment_summary() {
    ...
    echo -e "${GREEN}${BOLD}################################################################"
    echo "#                                                              #"
    echo "#     STAGING Kubernetes Cluster Deployment Completed!             #"
    echo "#                                                              #"
    echo "################################################################${NC}"
    echo
    ...
    echo "Cluster Name: STAGING Kubernetes Cluster"
    echo "Nodes: 2x Dell PowerEdge R740"
    ...
    echo "✓ High Availability (2-node control plane + worker)"
    ...
    echo "2. Set up staging SSL certificates"
    ...
    echo -e "${GREEN}${BOLD}🎉 Your STAGING Kubernetes cluster is ready for staging workloads! 🎉${NC}"
    echo
...
estimate_deployment_time() {
    ...
    echo "Phase 1: Server preparation - ~15 minutes per node (30 min total)"
    echo "Phase 2: Load balancer setup - ~5 minutes per node (10 min total)"
    echo "Phase 3: Cluster initialization - ~15 minutes"
    echo "Phase 4: Control plane joining - ~10 minutes (k8s-stg2 only)"
...
main() {
    ...
    log "Starting STAGING Kubernetes cluster master deployment..."
    ...
    success "STAGING Kubernetes cluster deployment orchestration completed!"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script orchestrates the complete deployment of a STAGING Kubernetes cluster"
        echo "across 2 Dell PowerEdge R740 servers."
        echo
        echo "Prerequisites:"
        echo "• All 2 servers have fresh Ubuntu 24.04 installation"
        echo "• SSH key authentication configured between servers"
        echo "• This script must be run from k8s-stg1"
        ...
        echo
        echo "Estimated deployment time: 2 hours"
        exit 0
        ;;
    *)
        main "$@"
