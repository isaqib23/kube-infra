...
# Run this script on k8s-dev1 after all control planes have joined
# Purpose: Configure distributed storage across all 4 Dell R740 servers
...
banner() {
    echo -e "${BLUE}"
    echo "=============================================================="
    echo "    Storage Setup - Single-Node Development Configuration"
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script configures storage for the single-node"
    echo "DEVELOPMENT Kubernetes cluster on k8s-dev1."
    echo
}
...
check_prerequisites() {
    ...
    # Check if node is ready (single-node cluster)
    local ready_nodes=$(kubectl get nodes --no-headers | grep -c "Ready")
    if [[ $ready_nodes -lt 1 ]]; then
        error "No ready nodes found. Expected 1 node for development cluster."
    fi

    success "Prerequisites check passed for single-node development cluster"
}

verify_all_nodes_storage() {
    log "Verifying storage directories on single-node cluster..."

    log "Checking storage directories on $SERVER_NAME ($SERVER_IP)..."

    # Check local storage directories
    if [[ -d "$STORAGE_BASE_PATH" ]]; then
        success "Storage directories verified on $SERVER_NAME"
    else
        warning "Storage directory $STORAGE_BASE_PATH does not exist. Will be created when needed."
    fi
}
    ...
    spec:
      template:
        spec:
          ...
          nodeSelector:
            kubernetes.io/hostname: k8s-dev1  # Run backup on k8s-dev1
EOF
...
show_completion_info() {
    ...
    kubectl get pv
    echo
    echo "=== Storage Distribution (Single-Node) ==="
    echo "Fast SSD Storage:"
    echo "  • PostgreSQL: 50Gi"
    echo "  • Redis: 20Gi"
    echo "  • Prometheus: 100Gi"
    echo "  • Grafana: 20Gi"
    echo "  • Loki: 100Gi"
    echo
    echo "Standard Storage:"
    echo "  • General Purpose: 200Gi"
    echo
    ...
    echo
    echo "=== Next Steps ==="
    echo "1. Deploy ingress controller: 06-ha-ingress-setup.sh"
    ...
    echo
    echo -e "${GREEN}Storage is now configured on the single-node development cluster!${NC}"
}
...
main() {
    ...
    log "Starting storage setup for single-node development cluster..."
    ...
    success "Storage setup completed successfully for single-node development cluster!"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script configures storage for the single-node DEVELOPMENT Kubernetes cluster."
        echo "Run this script on k8s-dev1 after cluster initialization."
        echo
        echo "Storage configuration includes:"
        echo "• Multiple storage classes (fast-ssd, standard, backup, logs)"
        echo "• Persistent volumes for applications"
