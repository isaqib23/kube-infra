...
# Run this script on k8s-dev1 after ingress setup
...
banner() {
    ...
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script deploys a comprehensive monitoring stack for"
    echo "the single-node development cluster on k8s-dev1."
    echo
}
...
spec:
  ...
  - name: kubernetes-ha-cluster.rules
    ...
    - alert: EtcdInsufficientMembers
      expr: count(up{job="kube-etcd"} == 1) < 1
      for: 3m
      labels:
        severity: critical
      annotations:
        summary: "etcd cluster has insufficient healthy members"
        description: "etcd cluster has {{ \$value }} healthy members. At least 1 is required for single-node."
    
    # HAProxy/Load balancer alerts (disabled for single-node - no HAProxy)
...
stringData:
  alertmanager.yml: |
    ...
    - name: 'critical-alerts'
      ...
      - url: 'http://localhost:9093/api/v1/alerts'
        send_resolved: true
        title: 'Critical Alert - DEVELOPMENT Kubernetes Cluster'
        text: |
          {{ range .Alerts }}
    ...
    - name: 'warning-alerts'
      ...
      - url: 'http://localhost:9093/api/v1/alerts'
        send_resolved: true
        title: 'Warning Alert - DEVELOPMENT Kubernetes Cluster'
        text: |
          {{ range .Alerts }}
          Alert: {{ .Annotations.summary }}
          Description: {{ .Annotations.description }}
...
install_additional_exporters() {
    ...
    # Note: HAProxy exporter skipped for single-node development (no HAProxy)
    success "Additional exporters check completed (HAProxy not needed for single-node)"
    return 0

    # HAProxy Exporter for load balancer monitoring (disabled for single-node)
  ...
  ha-cluster-overview.json: |
    {
      "dashboard": {
        "id": null,
        "title": "DEVELOPMENT Kubernetes Cluster Overview",
...
show_completion_info() {
    ...
    echo "✓ Prometheus (single instance)"
    echo "✓ Grafana (single instance)"
    echo "✓ AlertManager (single instance)"
    echo "✓ Loki Stack (single-binary mode)"
    echo "✓ Node Exporter"
    ...
    echo
    echo "=== Key Features ==="
    echo "• Monitoring for single-node development cluster"
    ...
    echo "• Custom dashboards for cluster overview"
    ...
    echo -e "${YELLOW}Remember to configure external alerting (email/Slack) for development use!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo "Run this script on k8s-dev1 after ingress setup."
        echo
        ...
        echo "• AlertManager (single instance)"
        echo "• Loki Stack (single-binary mode)"
        echo "• Node Exporter (metrics from node)"
        echo "• Custom alerting rules for cluster health"
