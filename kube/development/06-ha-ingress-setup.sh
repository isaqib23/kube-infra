...
# Run this script on k8s-dev1 after storage setup
...
banner() {
    ...
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script deploys a Single-Node Development NGINX Ingress"
  ...
  acme:
    ...
    - http01:
        ingress:
          class: nginx
---
# Let's Encrypt development Issuer
apiVersion: cert-manager.io/v1
kind: ClusterIssuer
metadata:
...
controller:
  # Single-Node Development - Deploy single replica
  replicaCount: 1
...
update_haproxy_for_ingress() {
    ...
    # Single-node development cluster - no HAProxy needed
    log "Single-node development cluster - HAProxy not used"
...
data:
  index.html: |
    ...
    <html>
    <head>
        <title>DEVELOPMENT Kubernetes Cluster Test</title>
    </head>
    <body>
        <h1>DEVELOPMENT Kubernetes Cluster - Ingress Test</h1>
        <p>Server: \$(hostname)</p>
        <p>Time: \$(date)</p>
...
show_completion_info() {
    ...
    echo
    echo "=== Service Access ==="
    echo "Add this entry to your local /etc/hosts file:"
    echo "$SERVER_IP grafana.${CLUSTER_DOMAIN} prometheus.${CLUSTER_DOMAIN} alertmanager.${CLUSTER_DOMAIN} test.${CLUSTER_DOMAIN}"
    echo
    ...
    echo
    echo -e "${GREEN}HA Ingress Controller is ready for development workloads!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo "Run this script on k8s-dev1 after storage setup."
        echo
        echo "Features deployed:"
        echo "• NGINX Ingress Controller with 4 replicas (HA)"
