...
# Run this script on k8s-cp1 after monitoring setup
...
banner() {
    ...
    echo "Kubernetes cluster across all 4 Dell R740 servers."
    echo
}
...
test_etcd_cluster() {
    ...
    if kubectl exec -n kube-system etcd-k8s-cp1 -- etcdctl \
        --endpoints=https://127.0.0.1:2379 \
        --cacert=/etc/kubernetes/pki/etcd/ca.crt \
        --cert=/etc/kubernetes/pki/etcd/server.crt \
        --key=/etc/kubernetes/pki/etcd/server.key \
...
generate_validation_report() {
    ...
    cat > "$report_file" << EOF
HA Kubernetes Cluster Validation Report
=======================================
Date: $(date)
Cluster: HA Kubernetes on 4x Dell R740 servers
...
show_completion_info() {
    ...
    if [[ $FAILED_TESTS -eq 0 ]]; then
        echo -e "${GREEN}🎉 ALL TESTS PASSED! Your HA cluster is ready for production!${NC}"
    elif [[ $FAILED_TESTS -le 2 ]]; then
        echo -e "${YELLOW}⚠ Minor issues detected. Review failed tests and address if needed.${NC}"
    else
        echo -e "${RED}❌ Multiple issues detected. Review and fix failed tests before production use.${NC}"
    fi
    ...
    echo "3. Configure production workloads"
    ...
    echo "□ SSL certificates for production domains"
    ...
    echo
    echo -e "${GREEN}Your HA Kubernetes cluster on 4x Dell R740 servers is validated and ready!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script performs comprehensive validation of the HA Kubernetes cluster."
        echo "Run this script on k8s-cp1 after all setup is complete."
        echo
        ...
        echo
        echo "The script generates a detailed report and provides production readiness guidance."
        exit 0
        ;;
    *)
        main "$@"
