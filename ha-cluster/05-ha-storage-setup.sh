...
# Run this script on k8s-cp1 after all control planes have joined
...
banner() {
    ...
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script configures distributed storage across all 4"
    echo "Dell R740 servers for the HA Kubernetes cluster."
    echo
}
...
check_prerequisites() {
    ...
    if [[ $ready_nodes -lt 4 ]]; then
        warning "Only $ready_nodes nodes are Ready. Expected 4 nodes. Continuing anyway..."
    fi
    ...
    spec:
      template:
        spec:
          ...
          nodeSelector:
            kubernetes.io/hostname: k8s-cp1  # Run backup on k8s-cp1
EOF
...
show_completion_info() {
    ...
    echo "  • PostgreSQL: 50Gi per node (200Gi total)"
    echo "  • Redis: 20Gi per node (80Gi total)"
    echo "  • Prometheus: 100Gi per node (400Gi total)"
    echo "  • Grafana: 20Gi per node (80Gi total)"
    echo "  • Loki: 100Gi per node (400Gi total)"
    echo
    echo "Standard Storage:"
    echo "  • General Purpose: 200Gi per node (800Gi total)"
    echo
    ...
    echo
    echo -e "${GREEN}Distributed storage is now configured across all 4 nodes!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script configures distributed storage for the HA Kubernetes cluster."
        echo "Run this script on k8s-cp1 after all control planes have joined."
        echo
        echo "Storage will be distributed across all 4 Dell R740 servers with:"
