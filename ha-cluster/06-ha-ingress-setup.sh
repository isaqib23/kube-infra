...
# Run this script on k8s-cp1 after storage setup
...
controller:
  # High Availability - Deploy on multiple nodes
  replicaCount: 4
...
data:
  index.html: |
    ...
    <html>
    <head>
        <title>HA Kubernetes Cluster Test</title>
    </head>
    <body>
        <h1>HA Kubernetes Cluster - Ingress Test</h1>
        <p>Server: \$(hostname)</p>
        <p>Time: \$(date)</p>
...
show_completion_info() {
    ...
    echo
    echo -e "${GREEN}HA Ingress Controller is ready for production workloads!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo "Run this script on k8s-cp1 after storage setup."
        echo
