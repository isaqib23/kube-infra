...
# Server Preparation Script for HA Kubernetes Cluster
# Run this script on ALL 4 Dell R740 servers
# Purpose: Prepare fresh Ubuntu 24.04 systems for Kubernetes HA deployment
...
LOG_FILE="/var/log/ha-k8s-prep.log"

# Server configuration - MODIFY THESE FOR EACH SERVER
...
banner() {
    echo -e "${BLUE}"
    echo "=============================================================="
    echo "    HA Kubernetes Server Preparation - Dell R740"
    echo "=============================================================="
    echo -e "${NC}"
    echo "This script will prepare a fresh Ubuntu 24.04 server for"
    echo "High Availability Kubernetes cluster deployment."
    echo
}
...
get_server_info() {
    ...
    for server in "${!SERVER_CONFIG[@]}"; do
        echo "  $server: ${SERVER_CONFIG[$server]}"
    done
    echo
    
    read -p "Enter this server's hostname (k8s-cp1, k8s-cp2, k8s-cp3, or k8s-cp4): " SERVER_NAME
    
    if [[ ! ${SERVER_CONFIG[$SERVER_NAME]+_} ]]; then
        error "Invalid server name. Must be one of: ${!SERVER_CONFIG[*]}"
    fi
    
    SERVER_IP="${SERVER_CONFIG[$SERVER_NAME]}"
    echo
...
create_directories() {
    ...
    # Create directories for HA setup
...
show_completion_info() {
    ...
    echo
    echo "=== Next Steps ==="
    echo "1. Run this script on all 4 servers (k8s-cp1, k8s-cp2, k8s-cp3, k8s-cp4)"
    echo "2. Run 02-ha-loadbalancer-setup.sh on all servers"
    echo "3. Run 03-ha-cluster-init.sh on k8s-cp1 (first control plane)"
    echo "4. Run 04-ha-cluster-join.sh on k8s-cp2, k8s-cp3, k8s-cp4"
    echo
    echo "=== Important Notes ==="
    echo "- SSH keys should be exchanged between all servers"
    echo "- Network connectivity verified between all nodes"
    echo "- Time synchronization is now active"
    echo "- Firewall configured for Kubernetes traffic"
    echo
    echo -e "${GREEN}Server $SERVER_NAME is ready for HA Kubernetes deployment!${NC}"
}
...
case "${1:-}" in
    --help|-h)
        ...
        echo
        echo "This script prepares a fresh Ubuntu 24.04 server for HA Kubernetes deployment."
        echo "Run this script on ALL 4 Dell R740 servers before cluster initialization."
        exit 0
        ;;
    *)
        main "$@"
